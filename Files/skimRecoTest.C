...
void skimRecoTest(Int_t iState = 1, Double_t lambdaTheta = 0, Double_t lambdaPhi = 0, Double_t lambdaThetaPhi = 0) {
	const char* inputFileName = Form("OniaTree_Y%dS_pThat2_HydjetDrumMB_miniAOD.root", iState);

	const char* outputFileName = Form("Y%dSReconstructedMCTestDataset_Lambda_Theta%.2f_Phi%.2f_ThetaPhi%.2f.root", iState, lambdaTheta, lambdaPhi, lambdaThetaPhi);
	...
	for (Long64_t iEvent = 0; iEvent < (totEntries); iEvent++) {
		...
		for (int iQQ = 0; iQQ < Reco_QQ_size; iQQ++) {
			...
			datasetCS.add(RooArgSet(centVar, eventWeightCSVar, massVar, yVar, ptVar, cosThetaLabVar, phiLabVar, etaLabMuplVar, etaLabMumiVar, cosThetaCSVar, phiCSVar, phiTildeCSVar, cosThetaHXVar, phiHXVar, phiTildeHXVar), polarWeightCS, errorWeightDown, errorWeightUp);
			datasetHX.add(RooArgSet(centVar, eventWeightHXVar, massVar, yVar, ptVar, cosThetaLabVar, phiLabVar, etaLabMuplVar, etaLabMumiVar, cosThetaCSVar, phiCSVar, phiTildeCSVar, cosThetaHXVar, phiHXVar, phiTildeHXVar), polarWeightHX, errorWeightDown, errorWeightUp);
...
void draw2DHist(const char* refFrameName = "CS" , Double_t lambdaTheta = 0, Double_t lambdaPhi = 0, Double_t lambdaThetaPhi = 0){
	...
	const char* FileName = Form("Y2SReconstructedMCTestDataset_Lambda_Theta%.2f_Phi%.2f_ThetaPhi%.2f.root", lambdaTheta, lambdaPhi, lambdaThetaPhi);
