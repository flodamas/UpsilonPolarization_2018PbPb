...
void testFullMCclosure(Int_t ptMin = 0, Int_t ptMax = 30, const char* refFrameName = "CS", Float_t cosThetaMin = -1, Float_t cosThetaMax = 1., Float_t phiMin = -180, Float_t phiMax = 180, Int_t iState = 1) {
	...
	auto* normFactor2D = productPDF2D.createIntegral(RooArgSet(cosTheta, phi), NormSet(RooArgSet(cosTheta, phi)), Range("PolaFitRange"));

	// 4. bind all components into a single variable

	RooGenericPdf compactPDF2D("compactPDF2D", " total polarization PDF", "(@0/@1)", {productPDF2D, *normFactor2D});
	compactPDF2D.setNormRange("PolaFitRange");

	auto compactNLL2D = compactPDF2D.createNLL(data, Range("PolaFitRange"));
	...
	RooMinimizer minimizer(*compactNLL2D);

	minimizer.setStrategy(1); // 1 for faster minimization, 2 for better convergence
	...
	savedLambdaTilde.setError(lambdaTilde.getPropagatedError(*compactFitResult));
...
*/
	...
	auto* testResult2D = compactPDF2D.fitTo(data, Save(), Range("PolaFitRange"), AsymptoticError(true), PrintLevel(0), RecoverFromUndefinedRegions(1000.), Offset("initial"));
