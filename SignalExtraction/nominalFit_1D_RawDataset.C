...
void scanNominalFit_lowPt_RawDataset() {
	...
	Float_t cosThetaEdges[21] = {-1., -0.8, -0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6, 0.8, 1.};
	...
	for (Int_t cosThetaIdx = 0; cosThetaIdx < numCosThetaEle - 1; cosThetaIdx++) {
		for (Int_t idx = 0; idx < numPhiEle - 1; idx++) {
			nominalFit_lowPt_RawDataset(ptEdges[5], ptEdges[6], kFALSE, cosThetaEdges[cosThetaIdx], cosThetaEdges[cosThetaIdx + 1], phiEdges[idx], phiEdges[idx + 1]);
		}
	}
}
