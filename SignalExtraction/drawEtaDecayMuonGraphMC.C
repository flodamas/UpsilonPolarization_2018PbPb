...
void drawEtaDecayMuonGraphMC(Int_t minPt = 0, Int_t maxPt = 30, Int_t centMin = 0, Int_t centMax = 90, Bool_t isCSframe = kFALSE, Float_t cosThetaMin = -1, Float_t cosThetaMax = 1, Int_t phiMin = -180, Int_t phiMax = 180,
						const char* filename = "../Files/MCUpsilonSkimmedWeightedDataset.root"){ //MC
	...
	// hEta -> GetYaxis() -> SetRangeUser(-190, 300);
	hEta -> GetZaxis() -> SetRangeUser(0, 10);
	gPad -> Update();
	CMS_lumi(cEta, "Weighted MC");

	cEta -> SaveAs(Form("EtaDecayMuonPlots/MC_HX_cent%dto%d_pt%dto%dGeV_cos%.1fto%.1f_phi%dto%d.png", centMin, centMax, minPt, maxPt, cosThetaMin, cosThetaMax, phiMin, phiMax), "RECREATE");
}

void drawEtaDecayMuonGraphMC_scan(){
	...
	for(int idx=0; idx< NumPhiArr-1; idx++){
		...
		drawEtaDecayMuonGraphMC(ptCuts[0], ptCuts[1], 0, 90, isCSframe, cosThetaCuts[5], cosThetaCuts[6], phiCuts[idx], phiCuts[idx+1],
						"../Files/MCUpsilonSkimmedWeightedDataset.root"); //MC
