...
TEfficiency* getAcceptance3DMap(const char* refFrameName, Double_t lambdaTheta = 0, Double_t lambdaPhi = 0, Double_t lambdaThetaPhi = 0) {
	...
	// TString fileName = Form("%s/AcceptanceResults%s_50M_kFNormal.root", AcceptanceResultsPath(gMuonAccName.Data()), "_fullPhi");
	// TString fileName = Form("%s/AcceptanceResults%s_10M_kFNormal.root", AcceptanceResultsPath(gMuonAccName.Data()), "_fullPhi");
...
TEfficiency* getEfficiency3DMap(const char* refFrameName, Double_t lambdaTheta = 0, Double_t lambdaPhi = 0, Double_t lambdaThetaPhi = 0) {
	...
	TString fileName = Form("%s/EfficiencyResults%s_kFNormal.root", EfficiencyResultsPath(gMuonAccName.Data()), "_fullPhi");
...
TH3D* getSysEff3DMap(const char* refFrameName, Double_t lambdaTheta = 0, Double_t lambdaPhi = 0, Double_t lambdaThetaPhi = 0) {
	...
	TString fileName = Form("%s/EfficiencyResults%s_kFNormal.root", EfficiencyResultsPath(gMuonAccName.Data()), "_fullPhi");
	// TString fileName = Form("%s/EfficiencyResults%s_noNColl.root", EfficiencyResultsPath(gMuonAccName.Data()), "_fullPhi");
 	// TString fileName = Form("%s/EfficiencyResults%s_100k.root", EfficiencyResultsPath(gMuonAccName.Data()), "_fullPhi");
...
RooArgSet extractPolarParam(TH2D* correctedHist, TString refFrameName = "CS",
                            ...
                            Bool_t isPhiFolded = kFALSE, Bool_t applyEff = kTRUE, int nLegendRows = 1) {
	...
	TCanvas* mc2DCanvas = draw2DMap(fitHist, refFrameName.Data(), nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, kTRUE, kFALSE, 1, isPhiFolded, nLegendRows);
	...
	resultTextRight->AddText(Form("#lambda_{#theta}  = %.3f #pm %.3f ", lambdaThetaVal, lambdaThetaErr));
	resultTextRight->AddText(Form("#lambda_{#theta#varphi} = %.3f #pm %.3f", lambdaThetaPhiVal, lambdaThetaPhiErr));
	resultTextRight->Draw();

	TPaveText* resultTextLeft = new TPaveText(0.61, 0.74, 0.87, 0.825, "NDC"); // Adjust coordinates
	...
	resultTextLeft->AddText(Form("#lambda_{#varphi} = %.3f #pm %.3f", lambdaPhiVal, lambdaPhiErr));
	resultTextLeft->AddText(Form("#tilde{#lambda}  = %.3f #pm %.3f", lambdaTildeVal, lambdaTildeErr));
	resultTextLeft->Draw();
...
void correctMC2DHist(TH2D* polarizedHist, TH2D* correctedHist, TString refFrameName = "CS",
	...
	if (!applyEff) {
		...
		accTotalCanvas = draw2DMap(hTotalCosThetaPhiAcc, refFrameName.Data(), nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, kFALSE, kFALSE, 1, isPhiFolded, nLegendRows);
		hTotalCosThetaPhiAcc->GetZaxis()->SetTitle("#varUpsilon(1S) acceptance denominator");
		display2DMapContents(hTotalCosThetaPhiAcc, nCosThetaBins, nPhiBins, kTRUE, 0.04, kBlack, 4);
		...
		for (Int_t iPhi = 0; iPhi < nPhiBins; iPhi++) {
			...
			if (applyAcc && applyEff) {
				if (acceptance == 0 || efficiency == 0) {
					...
					relEffUncHigh = effMapCosThetaPhi->GetEfficiencyErrorUp(iGlobalBin) * 5. / efficiency;
					relAccUncHigh = accMapCosThetaPhi->GetEfficiencyErrorUp(iGlobalBin) * 5. / acceptance;
				}
			}
	...
	if (drawPlot) {
		...
		display2DMapContents(correctedHist, nCosThetaBins, nPhiBins, kFALSE, 0.035, kBlack, 0);
...
void getPolarizedMCHist(TH2D* angDistHist2D, TString refFrameName = "CS",
	...
	const char* inputFileName = Form("../Files/Y1SReconstructedMCWeightedDataset_%s_Lambda_Theta%.2f_Phi%.2f_ThetaPhi%.2f_gen_noSF.root", gMuonAccName.Data(), lambdaTheta, lambdaPhi, lambdaThetaPhi);
	...
	TString rawDatasetName = Form("MCrawDataset%s", refFrameName.Data());

	RooDataSet* allDataset = (RooDataSet*)infile->Get(datasetName.Data());
	RooDataSet* rawDataset = (RooDataSet*)infile->Get(rawDatasetName.Data());
	...
	if (!rawDataset) {
		cout << "Raw dataset " << rawDatasetName << " not found. Check the dataset name." << endl;
		return;
	}
	...
	RooDataSet reducedRawDataset = *(RooDataSet*)rawDataset->reduce(kinematicCut);

	/// import the dataset to a workspace
	RooWorkspace wspace("workspace");
	wspace.import(reducedDataset);

	RooWorkspace wspaceRaw("workspaceRaw");
	wspaceRaw.import(reducedRawDataset);
	...
	RooRealVar cosThetaRaw = *wspaceRaw.var(CosThetaVarName(refFrameName.Data()));
	RooRealVar phiRaw = *wspaceRaw.var(PhiVarName(refFrameName.Data()));
	RooRealVar phiTildeRaw = *wspaceRaw.var(PhiTildeVarName(refFrameName.Data()));

	// cout << "Variables in the workspace: " << endl;
	// cosTheta.Print("V");
	// phi.Print("V");
	// phiTilde.Print("V");
	// cosThetaRaw.Print("V");
	...
	TH2D* angDistRawHist2D = new TH2D("angDistRawHist2D", ";cos #theta; #varphi (#circ); Number of generated #varUpsilon(1S) events", nCosThetaBins, cosThetaBinEdges[0], cosThetaBinEdges[nCosThetaBins], nPhiBins, phiBinEdges[0], phiBinEdges[nPhiBins]);
	...
	for (Int_t iEvent = 0; (ULong64_t)iEvent < totEvents; iEvent++) {
		// if (iEvent > 100) break;
		...
		angDistRawHist2D->Fill(cosThetaVal, phiVal);
	}

	/// fill the error histogram
	for (int iCosTheta = 1; iCosTheta <= nCosThetaBins; iCosTheta++) {
		for (int iPhi = 1; iPhi <= nPhiBins; iPhi++) {
			double binError = angDistRawHist2D->GetBinError(iCosTheta, iPhi) / angDistRawHist2D->GetBinContent(iCosTheta, iPhi) * angDistHist2D->GetBinContent(iCosTheta, iPhi) ; // relative error
			angDistHist2D->SetBinError(iCosTheta, iPhi, binError); // set the bin error to the raw histogram error
			// angDistHist2D->SetBinError(iCosTheta, iPhi, tempErrorUp[iCosTheta - 1][iPhi - 1]);
	...
	TCanvas* mc2DCanvas = draw2DMap(angDistHist2D, refFrameName.Data(), nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, kFALSE, kFALSE, 1, isPhiFolded, nLegendRows);
	display2DMapContents(angDistHist2D, nCosThetaBins, nPhiBins, kFALSE, 0.04, kWhite, 0); // values in the bins
	// display2DMapContents(angDistHist2D, nCosThetaBins, nPhiBins, kTRUE); // percentage error in the bins

	/// draw error histogram
	// draw2DMap(errorHist, refFrameName, nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, kFALSE, kFALSE, 1, isPhiFolded, nLegendRows);
	...
	// TCanvas* rawMC2DCanvas = draw2DMap(angDistRawHist2D, refFrameName.Data(), nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, kFALSE, kFALSE, 1, isPhiFolded, nLegendRows);
	// display2DMapContents(angDistRawHist2D, nCosThetaBins, nPhiBins, kTRUE, 0.04, kWhite, 4);
	...
	// /// Put texts inside the plot
	// legend2->DrawLatexNDC(.50, .88, "Reconstructed MC");
	// legend2->DrawLatexNDC(.50, .80, Form("#lambda_{#theta} = %.2f, #lambda_{#varphi} = %.2f, #lambda_{#theta#varphi} = %.2f", lambdaTheta, lambdaPhi, lambdaThetaPhi));
	...
	/// save the plot
	gSystem->mkdir("closureTest/HydjetAccEff", kTRUE);
	if (isPhiFolded)
		mc2DCanvas->SaveAs(Form("ClosureTest/HydjetAccEff/RecoMC_%s_pt%dto%d_cosTheta%.2fto%.2f_absphi%dto%d_LambdaTheta%.2f_Phi%.2f_ThetaPhi%.2f.png", refFrameName.Data(), ptMin, ptMax, cosThetaBinEdges[0], cosThetaBinEdges[nCosThetaBins], (int)phiBinEdges[0], (int)phiBinEdges[nPhiBins], lambdaTheta, lambdaPhi, lambdaThetaPhi), "RECREATE");
	else {
		/// pdf
		mc2DCanvas->SaveAs(Form("ClosureTest/HydjetAccEff/RecoMC_%s_pt%dto%d_cosTheta%.2fto%.2f_phi%dto%d_LambdaTheta%.2f_Phi%.2f_ThetaPhi%.2f.pdf", refFrameName.Data(), ptMin, ptMax, cosThetaBinEdges[0], cosThetaBinEdges[nCosThetaBins], (int)phiBinEdges[0], (int)phiBinEdges[nPhiBins], lambdaTheta, lambdaPhi, lambdaThetaPhi), "RECREATE");
		/// png
		mc2DCanvas->SaveAs(Form("ClosureTest/HydjetAccEff/RecoMC_%s_pt%dto%d_cosTheta%.2fto%.2f_phi%dto%d_LambdaTheta%.2f_Phi%.2f_ThetaPhi%.2f.png", refFrameName.Data(), ptMin, ptMax, cosThetaBinEdges[0], cosThetaBinEdges[nCosThetaBins], (int)phiBinEdges[0], (int)phiBinEdges[nPhiBins], lambdaTheta, lambdaPhi, lambdaThetaPhi), "RECREATE");
	}
...
void getGENMCHist(TH2D* angDistHist2D, TString refFrameName = "CS",
                  ...
                  Int_t nCosThetaBins = 5, const vector<Double_t>& cosThetaBinEdges = {},
                  Int_t nPhiBins = 6, const vector<Double_t>& phiBinEdges = {},
                  Bool_t applyAcc = kTRUE, Bool_t isPhiFolded = kFALSE, int nLegendRows = 1) {
	writeExtraText = true; // if extra text
	extraText = "       Internal";
	// extraText = "       Simulation Preliminary";
	...
	/// draw polarized MC histogram
	TCanvas* mc2DCanvas = draw2DMap(angDistHist2D, refFrameName.Data(), nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, kFALSE, kFALSE, 1, isPhiFolded, nLegendRows);
	display2DMapContents(angDistHist2D, nCosThetaBins, nPhiBins, kFALSE); // values in the bins
...
void closureTest(TString refFrameName = "CS",
	...
	if (!applyEff)
		getGENMCHist(angDistHist2D, refFrameName, lambdaTheta0, lambdaPhi0, lambdaThetaPhi0, ptMin, ptMax, nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, applyAcc, isPhiFolded, nLegendRows); /// apply polarization to NoGENFilter sample
	else
		getPolarizedMCHist(angDistHist2D, refFrameName, lambdaTheta0, lambdaPhi0, lambdaThetaPhi0, ptMin, ptMax, nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, isPhiFolded, nLegendRows); /// apply polarization to recoMC sample
	...
	for (int iItr = 0; iItr <= totNItrs; iItr++) {
		/// correct the MC
		correctMC2DHist(angDistHist2D, correctedHist, refFrameName, polarParams, ptMin, ptMax, nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, kTRUE, isPhiFolded, applyAcc, applyEff, lambdaTheta0, lambdaPhi0, lambdaThetaPhi0, nLegendRows);

		/// due to drawing issue, separate correctedHist and fittedHist
		correctMC2DHist(angDistHist2D, fittedHist, refFrameName, polarParams, ptMin, ptMax, nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, kFALSE, isPhiFolded, applyAcc, applyEff, lambdaTheta0, lambdaPhi0, lambdaThetaPhi0, nLegendRows);

		/// extract parameters from the corrected hist
		polarParams = extractPolarParam(fittedHist, refFrameName, ptMin, ptMax, nCosThetaBins, cosThetaBinEdges, nPhiBins, phiBinEdges, isPhiFolded, applyEff, nLegendRows);
