...
void fitMCSignalShape_twoCB(Int_t minPt = 0, Int_t maxPt = 30, Int_t centMin = 0, Int_t centMax = 90) {
	...
	// fit
	auto* fitResult = signal.fitTo(data, Save(), Extended(true), PrintLevel(-1), Minos(false), NumCPU(3), Range(massMin, massMax), SumW2Error(true)); // quoting RooFit: "sum-of-weights and asymptotic error correction do not work with MINOS errors"
	...
	canvas->SaveAs(Form("Figures/twoCB_cent%dto%d_pt%dto%dGeV.png", centMin, centMax, minPt, maxPt), "RECREATE");
}
