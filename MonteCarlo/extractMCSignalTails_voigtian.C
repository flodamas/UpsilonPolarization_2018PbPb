...
void extractMCSignalTails_voigtian(Int_t centMin = 0, Int_t centMax = 90, Int_t ptMin = 0, Int_t ptMax = 30, const char* filename = "../Files/Y1SReconstructedMCWeightedDataset_TriggerAcc_Lambda_Theta0.00_Phi0.00_ThetaPhi0.00.root", Bool_t isCSframe = kFALSE, Float_t cosThetaMin = -1, Float_t cosThetaMax = 1, Float_t phiMin = -180, Float_t phiMax = 180, bool saveParams = true) {
	...
	const char* signalShapeName = "Voigtian";
	...
	// fit
	auto* fitResult = VoigtianFit(wspace, massDataset, massMin, massMax);

	RooRealVar mean = *wspace.var(Form("mean%s", signalShapeName));
	RooRealVar sigma = *wspace.var(Form("sigma%s", signalShapeName));
	RooRealVar width = *wspace.var(Form("width%s", signalShapeName));
	...
	if (saveParams) {
		...
		RooArgSet* Params = new RooArgSet(sigma, width);
...
void scanExtractMCSignalTails_voigtian(Bool_t isCSframe = kFALSE) {
	...
	for (Int_t ptIdx = 0; ptIdx < NPtBins; ptIdx++) {
		for (Int_t cosThetaIdx = 0; cosThetaIdx < NumCosThetaEle - 1; cosThetaIdx++) {
			for (Int_t phiIdx = 0; phiIdx < NumPhiEle - 1; phiIdx++) {
				extractMCSignalTails_voigtian(gCentralityBinMin, gCentralityBinMax, gPtBinning[ptIdx], gPtBinning[ptIdx + 1], "../Files/Y1SReconstructedMCWeightedDataset_TriggerAcc_Lambda_Theta0.00_Phi0.00_ThetaPhi0.00.root", isCSframe, cosThetaEdges[cosThetaIdx], cosThetaEdges[cosThetaIdx + 1], phiEdges[phiIdx], phiEdges[phiIdx + 1], true);
			}
		}
	}
}
