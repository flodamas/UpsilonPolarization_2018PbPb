...
void acceptanceMap_GenWeights(Int_t iState = 1, Int_t ptMin = 0, Int_t ptMax = 30) {  
	...
	double weights_test1[2][2] = {{0.88, -0.99}, {0.00, -0.50}}; 
	double weights_test2[2][2] = {{-1.00, 0.00}, {-1.00, 0.00}}; 
	// cout << weights_test1[0][0] << endl;
	...
	double weights_start1[2][2] = {{1.00, 1.00}, {1.00, 1.00}}; 
	double weights_start2[2][2] = {{0.00, 0.00}, {0.00, 0.00}}; 
	double weights_start3[2][2] = {{1.00, 0.00}, {1.00, 0.00}}; 
	double weights_start4[2][2] = {{0.00, -0.50}, {0.00, -0.50}}; 
	// cout << weights_sart1[0][0] << endl;


	// // ******** Set lambda_theta and lambda_phi values to be used for the acceptance plots ******** //

	const int NumIdx = 1; //number of test values for lambda_theta and lambda_phi

	// double lambda_theta[NumIdx] = {-1, -0.7, -0.5, -0.2, 0, 0.2, 0.5, 0.7, 1};
	// // double lambda_theta = 1;
	// double lambda_phi[NumIdx] = {-1, -0.7, -0.5, -0.2, 0, 0.2, 0.5, 0.7, 1};
	...
	for(int idx=0; idx<NumIdx; idx++){
		...
		sprintf(hHXNumName, "hHXNum%d", idx);
		sprintf(hHXDenName, "hHXDen%d", idx);
		sprintf(hHXAccpName, "hHXAccp%d", idx);
		sprintf(cHXNumName, "cHXNum%d", idx);
		sprintf(cHXDenName, "cHXDen%d", idx);
		sprintf(cHXAccpName, "cHXAccp%d", idx);
		...
		sprintf(hCSNumName, "hCSNum%d", idx);
		sprintf(hCSDenName, "hCSDen%d", idx);
		sprintf(hCSAccpName, "hCSAccp%d", idx);
		sprintf(cCSNumName, "cCSNum%d", idx);
		sprintf(cCSDenName, "cCSDen%d", idx);
		sprintf(cCSAccpName, "cCSAccp%d", idx);
		...
		HXCSNTuple -> Draw(Form("muplPhiHX:muplCosThetaHX>>hHXNum%d", idx), Form("(1 + %f*pow(muplCosThetaHX, 2.0) + %f*(1- pow(muplCosThetaHX, 2.0) * cos(2*muplPhiHX)))*%s", weights_start1[0][0], weights_start1[0][1], SingleMuonAccCuts), "COLZ");
		...
		legend -> DrawLatexNDC(.48, .78, Form("#lambda_{#theta, HX} = %.2f, #lambda_{#phi, HX} = %.2f", weights_start1[0][0], weights_start1[0][1]));
		...
		// (Save the canvas)
		cHXNum[idx] -> SaveAs(Form("AcceptanceMaps/%dS/withWeights/HXNum_Theta%.2f_Phi%.2f_pt%dto%dGeV.png", iState, weights_start1[0][0], weights_start1[0][1], ptMin, ptMax), "RECREATE");
		...
		HXCSNTuple -> Draw(Form("muplPhiHX:muplCosThetaHX>>hHXDen%d",idx), Form("(1+ %f*pow(muplCosThetaHX, 2.0)+ %f*(1- pow(muplCosThetaHX, 2.0) * cos(2*muplPhiHX)))", weights_start1[0][0], weights_start1[0][1]), "COLZ");
		// Put text upper right conner of the plot
		...
		cHXDen[idx] -> SaveAs(Form("AcceptanceMaps/%dS/withWeights/HXDen_Theta%.2f_Phi%.2f_pt%dto%dGeV.png", iState, weights_start1[0][0], weights_start1[0][1], ptMin, ptMax), "RECREATE");
		...
		// (apply weights, cuts and draw 2D graph on the phi-costheta phase space)
		HXCSNTuple -> Draw(Form("muplPhiCS:muplCosThetaCS>>hCSNum%d", idx), Form("(1 + %f*pow(muplCosThetaCS, 2.0) + %f*(1- pow(muplCosThetaCS, 2.0) * cos(2*muplPhiCS)))*%s", weights_start1[1][0], weights_start1[1][1], SingleMuonAccCuts), "COLZ");
		// Put text upper right conner of the plot
		...
		legend -> DrawLatexNDC(.48, .90, Form("|y^{#mu#mu}| < 2.4, %d < p_{T}^{#mu#mu} < %d GeV", ptMin, ptMax));
		legend -> DrawLatexNDC(.48, .84, Form("#varUpsilon(%dS) acc. for |#eta^{#mu}| < 2.4, p_{T}^{#mu} > 3.5 GeV", iState));
		legend -> DrawLatexNDC(.48, .78, Form("#lambda_{#theta, CS} = %.2f, #lambda_{#phi, CS} = %.2f", weights_start1[1][0], weights_start1[1][1]));
		...
		// (Save the canvas)
		cCSNum[idx] -> SaveAs(Form("AcceptanceMaps/%dS/withWeights/CSNum_Theta%.2f_Phi%.2f_pt%dto%dGeV.png", iState, weights_start1[1][0], weights_start1[1][1], ptMin, ptMax), "RECREATE");
		...
		HXCSNTuple -> Draw(Form("muplPhiCS:muplCosThetaCS>>hCSDen%d",idx), Form("(1+ %f*pow(muplCosThetaCS, 2.0)+ %f*(1- pow(muplCosThetaCS, 2.0) * cos(2*muplPhiCS)))", weights_start1[1][0], weights_start1[1][1]), "COLZ");
		// Put text upper right conner of the plot
		...
		cCSDen[idx] -> SaveAs(Form("AcceptanceMaps/%dS/withWeights/CSDen_Theta%.2f_Phi%.2f_pt%dto%dGeV.png", iState, weights_start1[1][0], weights_start1[1][1], ptMin, ptMax), "RECREATE");
		...
		cHXAccp[idx] -> SaveAs(Form("AcceptanceMaps/%dS/withWeights/HXAccp_Theta%.2f_Phi%.2f_pt%dto%dGeV.png", iState, weights_start1[0][0], weights_start1[0][1], ptMin, ptMax), "RECREATE");
		...
		cCSAccp[idx] -> SaveAs(Form("AcceptanceMaps/%dS/withWeights/CSAccp_Theta%.2f_Phi%.2f_pt%dto%dGeV.png", iState, weights_start1[1][0], weights_start1[1][1], ptMin, ptMax), "RECREATE");
