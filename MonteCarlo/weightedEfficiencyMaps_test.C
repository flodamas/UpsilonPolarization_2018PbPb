...
void weightedEfficiencyMaps_test(Int_t ptMin = 0, Int_t ptMax = 2, TString muonAccName = "UpsilonTriggerThresholds", Float_t lambdaTheta = 0, Float_t lambdaPhi = 0, Float_t lambdaThetaPhi = 0, Bool_t isPhiFolded = kTRUE, Int_t iState = gUpsilonState) {
	...
	for (Long64_t iEvent = 0; iEvent < totEntries; iEvent++) {
		...
		for (int iGen = 0; iGen < Gen_QQ_size; iGen++) {
			...
			if (isRecoMatched) {
                ...
                /// numerator for the efficiency
				allGood = firesTrigger && isRecoMatched && dimuonMatching && goodVertexProba && trackerAndGlobalMuons && hybridSoftMuons;
	...
	const char* outputFileName = Form("%s/EfficiencyResults%s_noW.root", path, isPhiFolded ? "" : "_fullPhi");
