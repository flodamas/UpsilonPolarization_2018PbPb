...
/// draw pulls for 1D histograms
TPad* drawPulls(TH1D* positiveHist, TH1D* negativeHist, const char* xTitle, bool isCosTheta = kTRUE) {
	...
	/// Get the number of bins and the range of the histogram
	int nBins = positiveHist->GetNbinsX();
	double xmin = positiveHist->GetXaxis()->GetBinLowEdge(1);
	double xmax = positiveHist->GetXaxis()->GetBinUpEdge(nBins);
	...
	for (int iBin = 0; iBin < nBins; iBin++) {
		...
		double positiveEff = positiveHist->GetBinContent(iBin + 1);
		double negativeEff = negativeHist->GetBinContent(iBin + 1);

		double positiveEffErr = positiveHist->GetBinError(iBin + 1);
		double negativeEffErr = negativeHist->GetBinError(iBin + 1);

		double xpoint = positiveHist->GetXaxis()->GetBinCenter(iBin + 1);
		// cout << xpoint << " " << positiveEff << " " << negativeEff << " " << positiveEffErrUp << " " << negativeEffErrUp << " " << positiveEffErrDown << " " << negativeEffErrDown << endl;
		...
		if (positiveEff > negativeEff) {
			pull = (positiveEff - negativeEff) / sqrt(positiveEffErr * positiveEffErr + negativeEffErr * negativeEffErr);
			pullerr = sqrt(pow(positiveEffErr / sqrt(pow(positiveEffErr, 2) + pow(negativeEffErr, 2)), 2) + pow(negativeEffErr / sqrt(pow(positiveEffErr, 2) + pow(negativeEffErr, 2)), 2));
		}
...
std::vector<std::vector<TEfficiency*>> accEffplots_3Dto1D(Int_t ptMin = 0, Int_t ptMax = 30, const char* refFrameName = "CS", const Int_t nCosThetaBins = 5, Double_t cosThetaMin = -0.7, Double_t cosThetaMax = 0.7, const Int_t nPhiBins = 5, Int_t phiMin = -180, Int_t phiMax = 180, Int_t iState = gUpsilonState, Bool_t isPhiFolded = kFALSE, TString accName = "MuonUpsilonTriggerAcc") { // accName = "MuonSimpleAcc", "MuonWithin2018PbPbAcc", or "MuonUpsilonTriggerAcc"
	...
	DrawEfficiency1DHist(accMapCosTheta, ptMin, ptMax, iState, kTRUE, kTRUE, refFrameName, MuonAccName.Data(), isPhiFolded);
	DrawEfficiency1DHist(accMapPhi, ptMin, ptMax, iState, kTRUE, kFALSE, refFrameName, MuonAccName.Data(), isPhiFolded);

	/// draw efficiency 1D histograms
	DrawEfficiency1DHist(effMapCosTheta, ptMin, ptMax, iState, kFALSE, kTRUE, refFrameName, MuonAccName.Data(), isPhiFolded);
	DrawEfficiency1DHist(effMapPhi, ptMin, ptMax, iState, kFALSE, kFALSE, refFrameName, MuonAccName.Data(), isPhiFolded);
...
void accEffDenplots_3Dto1D_comparison(Int_t ptMin = 2, Int_t ptMax = 6, const char* refFrameName = "CS", const Int_t nCosThetaBins = 5, Double_t cosThetaMin = -0.7, Double_t cosThetaMax = 0.7, const Int_t nPhiBins = 1, Int_t phiMin = 0, Int_t phiMax = 60, Int_t iState = gUpsilonState, Bool_t isPhiFolded = kFALSE, Bool_t isCosTheta = kTRUE, TString accName = "MuonUpsilonTriggerAcc", Bool_t isDen = kTRUE) { // accName = "MuonSimpleAcc", "MuonWithin2018PbPbAcc", or "MuonUpsilonTriggerAcc"
	...
	if (isCosTheta) cout << "Drawing acceptance x efficiency, acc, eff 1D histograms for |phi| range " << phiMin << " to " << phiMax << " degrees..." << endl;
	else cout << "Drawing acceptance x efficiency, acc, eff 1D histograms for |cosTheta| range " << cosThetaMin << " to " << cosThetaMax << "..." << endl;
	...
	cout << "[positive Hist] " << "ptMin: " << ptMin << ", ptMax: " << ptMax << ", nCosTheta: " << nCosThetaBins << ", cosThetaMin: " << cosThetaMin << ", cosThetaMax: " << cosThetaMax << ", nPhiBins: " << nPhiBins << ", phiMin: " << phiMin << ", phiMax: " << phiMax << endl;
	cout << isCosTheta << endl;
	...
	std::vector<TH1D*> positiveDenHist = {nullptr, nullptr, nullptr};
	std::vector<TH1D*> negativeDenHist = {nullptr, nullptr, nullptr};
	...
	for (int plotTypeBin = 0; plotTypeBin < 3; plotTypeBin++) {
		...
		/// draw acceptance x efficiency 1D histograms for positive and negative phi (pad1)
		comparisonCanvas[plotTypeBin] = new TCanvas(Form("comparisonCanvas%d", plotTypeBin), Form("comparisonCanvas%d", plotTypeBin), 600, 600);
		...
		if (isDen == kTRUE) {
			if (plotTypeBin == 0) frameHist1D[plotTypeBin]->GetYaxis()->SetTitle("Acceptance Denominator");
			else if (plotTypeBin == 1) frameHist1D[plotTypeBin]->GetYaxis()->SetTitle("Efficiency Denominator");
			else frameHist1D[plotTypeBin]->GetYaxis()->SetTitle("Acceptance x Efficiency Denominator");
		}

		else {
			if (plotTypeBin == 0) frameHist1D[plotTypeBin]->GetYaxis()->SetTitle("Acceptance Numerator");
			else if (plotTypeBin == 1) frameHist1D[plotTypeBin]->GetYaxis()->SetTitle("Efficiency Numerator");
			else frameHist1D[plotTypeBin]->GetYaxis()->SetTitle("Acceptance x Efficiency Numerator");
		}

		/// draw kinematics legend
		TLatex legend;
		legend.SetTextAlign(22);
		legend.SetTextSize(0.05);
		legend.DrawLatexNDC(.55, .85, Form("%s < 2.4, %s", gDimuonRapidityVarTitle, DimuonPtRangeText(ptMin, ptMax)));
		...
		/// get the denominator or numerator histograms
		if (isDen == kTRUE) {
			positiveDenHist[plotTypeBin] = (TH1D*) positiveHist[plotTypeBin]->GetTotalHistogram();
			negativeDenHist[plotTypeBin] = (TH1D*) negativeHist[plotTypeBin]->GetTotalHistogram();
		}

		else{
			positiveDenHist[plotTypeBin] = (TH1D*) positiveHist[plotTypeBin]->GetPassedHistogram();
			negativeDenHist[plotTypeBin] = (TH1D*) negativeHist[plotTypeBin]->GetPassedHistogram();
		}
		...
		positiveDenHist[plotTypeBin]->SetMarkerColor(TColor::GetColor("#A559AA"));
		positiveDenHist[plotTypeBin]->SetMarkerStyle(20);
		positiveDenHist[plotTypeBin]->SetLineColor(TColor::GetColor("#A559AA"));
		
		negativeDenHist[plotTypeBin]->Draw("PE same");
		negativeDenHist[plotTypeBin]->SetMarkerColor(TColor::GetColor("#009ADE"));
		negativeDenHist[plotTypeBin]->SetMarkerStyle(24);
		negativeDenHist[plotTypeBin]->SetLineColor(TColor::GetColor("#009ADE"));

		/// draw cosTheta or phi range legend
		TLegend* legend1D = new TLegend(0.17, 0.58, 0.4, 0.73);
		legend1D->SetBorderSize(0);
		legend1D->SetFillStyle(0);

		if (isCosTheta == kTRUE) {
			legend1D->AddEntry(positiveDenHist[plotTypeBin], Form("%d < #varphi < %d#circ", phiMin, phiMax), "lep");
			legend1D->AddEntry(negativeDenHist[plotTypeBin], Form("%d < #varphi < %d#circ", -phiMax, -phiMin), "lep");
		}
		else {
			legend1D->AddEntry(positiveDenHist[plotTypeBin], Form("%.2f < cos#theta < %.2f", cosThetaMin, cosThetaMax), "lep");
			legend1D->AddEntry(negativeDenHist[plotTypeBin], Form("%.2f < cos#theta < %.2f", -cosThetaMax, -cosThetaMin), "lep");
		}
		legend1D->Draw();
		...
		TPad* bottomPad = drawPulls(positiveDenHist[plotTypeBin], negativeDenHist[plotTypeBin], xTitle.Data(), isCosTheta);
		...
		if (isDen == kTRUE) {
			if (isCosTheta == kTRUE) { 
				if (plotTypeBin == 0) comparisonCanvas[plotTypeBin]->SaveAs(Form("AcceptanceMaps/%dS/analysisBin/1DAccComp_Den%s%s%s_pt%dto%d_absPhi%dto%d_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, phiMin, phiMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
				else if (plotTypeBin == 1) comparisonCanvas[plotTypeBin]->SaveAs(Form("EfficiencyMaps/%dS/analysisBin/1DEffComp_Den%s%s%s_pt%dto%d_absPhi%dto%d_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, phiMin, phiMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
				else if (plotTypeBin == 2) comparisonCanvas[plotTypeBin]->SaveAs(Form("AccxEffMaps/%dS/analysisBin/1DAccxEffComp_Den%s%s%s_pt%dto%d_absPhi%dto%d_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, phiMin, phiMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
			}
			
			else {
				if (plotTypeBin == 0) comparisonCanvas[plotTypeBin]->SaveAs(Form("AcceptanceMaps/%dS/analysisBin/1DAccComp_Den%s%s%s_pt%dto%d_absCosTheta%.2fto%.2f_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, cosThetaMin, cosThetaMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
				else if (plotTypeBin == 1) comparisonCanvas[plotTypeBin]->SaveAs(Form("EfficiencyMaps/%dS/analysisBin/1DEffComp_Den%s%s%s_pt%dto%d_absCosTheta%.2fto%.2f_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, cosThetaMin, cosThetaMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
				else if (plotTypeBin == 2) comparisonCanvas[plotTypeBin]->SaveAs(Form("AccxEffMaps/%dS/analysisBin/1DAccxEffComp_Den%s%s%s_pt%dto%d_absCosTheta%.2fto%.2f_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, cosThetaMin, cosThetaMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
			}
		}

		else {
			if (isCosTheta == kTRUE) { 
				if (plotTypeBin == 0) comparisonCanvas[plotTypeBin]->SaveAs(Form("AcceptanceMaps/%dS/analysisBin/1DAccComp_Num%s%s%s_pt%dto%d_absPhi%dto%d_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, phiMin, phiMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
				else if (plotTypeBin == 1) comparisonCanvas[plotTypeBin]->SaveAs(Form("EfficiencyMaps/%dS/analysisBin/1DEffComp_Num%s%s%s_pt%dto%d_absPhi%dto%d_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, phiMin, phiMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
				else if (plotTypeBin == 2) comparisonCanvas[plotTypeBin]->SaveAs(Form("AccxEffMaps/%dS/analysisBin/1DAccxEffComp_Num%s%s%s_pt%dto%d_absPhi%dto%d_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, phiMin, phiMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
			}
			
			else {
				if (plotTypeBin == 0) comparisonCanvas[plotTypeBin]->SaveAs(Form("AcceptanceMaps/%dS/analysisBin/1DAccComp_Num%s%s%s_pt%dto%d_absCosTheta%.2fto%.2f_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, cosThetaMin, cosThetaMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
				else if (plotTypeBin == 1) comparisonCanvas[plotTypeBin]->SaveAs(Form("EfficiencyMaps/%dS/analysisBin/1DEffComp_Num%s%s%s_pt%dto%d_absCosTheta%.2fto%.2f_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, cosThetaMin, cosThetaMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
				else if (plotTypeBin == 2) comparisonCanvas[plotTypeBin]->SaveAs(Form("AccxEffMaps/%dS/analysisBin/1DAccxEffComp_Num%s%s%s_pt%dto%d_absCosTheta%.2fto%.2f_%s.png", iState, positiveDenHist[plotTypeBin]->GetName(), refFrameName, MuonAccName.Data(), ptMin, ptMax, cosThetaMin, cosThetaMax, isPhiFolded ? "folded" : "fullPhi"), "RECREATE");
			}
		}
		...
		// delete comparisonCanvas[plotTypeBin];
		// delete frameHist1D[plotTypeBin];
		// delete positiveDenHist[plotTypeBin];
		// delete negativeDenHist[plotTypeBin];
	}
	return;
}

void scan_accEffDenplots_3Dto1D_comparison(Bool_t isDen = kTRUE, Bool_t isPhiFolded = kTRUE, TString accName = "MuonUpsilonTriggerAcc"){

	// for (int ptBin = 0; ptBin < NPtBins; ++ptBin) {
	for (int ptBin = 1; ptBin < NPtBins; ++ptBin) {
		for (int phiBin = 0; phiBin < (isPhiFolded ? NPhiBins: NFullPhiBins); ++phiBin) {
			for (int refFrame = 0; refFrame < 2; ++refFrame) {
				if (isPhiFolded) accEffDenplots_3Dto1D_comparison(gPtBinning[ptBin], gPtBinning[ptBin + 1], refFrame == 0 ? "CS" : "HX", 5, -0.7, 0.7, 1, gPhiBinning[phiBin], gPhiBinning[phiBin + 1], gUpsilonState, isPhiFolded, kTRUE, accName, isDen);
				else accEffDenplots_3Dto1D_comparison(gPtBinning[ptBin], gPtBinning[ptBin + 1], refFrame == 0 ? "CS" : "HX", 5, -0.7, 0.7, 1, gFullPhiBinning[phiBin], gFullPhiBinning[phiBin + 1], gUpsilonState, isPhiFolded, kTRUE, accName, isDen);
			}
		}
	}	
	...
	for (int ptBin = 1; ptBin < NPtBins; ++ptBin) {
		for (int cosThetaBin = 3; cosThetaBin < NCosThetaBins; ++cosThetaBin) {
			for (int refFrame = 0; refFrame < 2; ++refFrame) {
				accEffDenplots_3Dto1D_comparison(gPtBinning[ptBin], gPtBinning[ptBin + 1], refFrame == 0 ? "CS" : "HX", 1, gCosThetaBinning[cosThetaBin], gCosThetaBinning[cosThetaBin + 1], 6, -180, 180, gUpsilonState, isPhiFolded, kFALSE, accName, isDen);
			}
		}
	}
}