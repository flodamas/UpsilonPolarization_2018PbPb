...
void UpsilonHXFrame3() { //version3 (In this version, I tried to rotate the coordinate to production plane and boost to the quarkonium rest frame) 
	...
	for(int EveNum=0; EveNum<(totEntries); EveNum++){
		...
		if(EveNum%100000==0){
			cout << "********* " << (double)(100.*EveNum/(totEntries)) << "% completed" << " ********" << endl;
		}
		...
		for(int QQEveNum=0; QQEveNum<Reco_QQ_size; QQEveNum++){
			...
			if( // ((HLTriggers&(ULong64_t)(1<<(Bits[SelectedBit]-1)))==(ULong64_t)(1<<(Bits[SelectedBit]-1)))
				...
				// ******** Construct 4-momentum vector of upsilon, muons, and beams (Lab Frame) ******** //
				...
				//(4-Momentum of the beams will be used later for the CS frame)
				...
				// ******** Rotate the lab frame so that z-axis heads to upsilon's momentum direction and x-axis is on the production plane ******** //
				// (Note. TVector3.Ratate() rotates the vectors not the coordinates, so should rotate -phi and -theta)
				muplPvecLab.RotateZ(-upsPvecLab.Phi());
				muplPvecLab.RotateY(-upsPvecLab.Theta());
				mumiPvecLab.RotateZ(-upsPvecLab.Phi());
				mumiPvecLab.RotateY(-upsPvecLab.Theta());

				beam1PvecLab.RotateZ(-upsPvecLab.Phi());
				beam1PvecLab.RotateY(-upsPvecLab.Theta());
				beam2PvecLab.RotateZ(-upsPvecLab.Phi());
				beam2PvecLab.RotateY(-upsPvecLab.Theta());

				upsPvecLab.RotateZ(-upsPvecLab.Phi());
				upsPvecLab.RotateY(-upsPvecLab.Theta());

				// ******** Rotate the lab frame so that z-axis heads to upsilon's momentum direction and x-axis is on the production plane ******** //
				ups4MomLab.SetX(upsPvecLab.Px());
				ups4MomLab.SetY(upsPvecLab.Py());
				ups4MomLab.SetZ(upsPvecLab.Pz());
				...
				HXNTuple -> Fill(mupl4MomBoosted.CosTheta(), mupl4MomBoosted.Phi());


				// ******** HX to CS (rotation from HX frame to CS frame) ******** //
				// (1. Rotate the lab frame coordinate so that z-axis heads to the quarkonium momentum direction and x-axis on the production plane)
				// (2. Boost two beams to upsilon's rest frame)
				// (3. Get angle between between b1 and ZHX in the upsilon's rest frame, and two beams(b1 and -b2))
				...
				TVector3 muplPvecBoostedCS(mupl4MomBoosted.Px(), mupl4MomBoosted.Py(), mupl4MomBoosted.Pz());	
				TVector3 mumiPvecBoostedCS(mumi4MomBoosted.Px(), mumi4MomBoosted.Py(), mumi4MomBoosted.Pz());	
	...
	TFile* file = new TFile("Upsilon1S_Reference_Frames3.root","RECREATE","Upsilon 1S"); 
